...
function useLocalStorageEventListener(callback) {
 ...
 */
function readLocalStorageValue(key, initialValue, serializer) {
  if (typeof window === "undefined") {
    return initialValue;
  }
 ...
 */
function writeLocalStorageValue(key, value, serializer) {
  if (typeof window === "undefined") {
    return;
  }
...
export function useLocalStorage(key, initialValue, options = {}) {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("local-storage", handleCustomEvent);
  }, [key, syncData]);
