...
<Callout title="Why Use This Hook?">
  - ✅ **Complete State Management** - Handles loading, error, and data states -
  ✅ **Automatic Cleanup** - Prevents memory leaks and race conditions - ✅
  **Flexible Execution** - Immediate or manual execution - ✅ **Error Handling**
  - Built-in error catching and reporting - ✅ **TypeScript Ready** - Full type
...
```bash
react-usekit init
# Select "hooks" → Choose language → Select "useAsync"
```
...
function UserProfile({ userId }: { userId: number }) {
  const {
    ...
    execute,
    reset,
  } = useAsync<User>(() => fetch(`/api/users/${userId}`).then(res => res.json()), {
    immediate: true,
  });
...
function UserProfile({ userId }) { const { data: user, loading, error, execute, reset } = useAsync(
() => fetch(`/api/users/${userId}`).then(res => res.json()), { immediate: true } );

if (loading) return <div>Loading user...</div>; if (error) return <div>Error: {error.message}</div>;
if (!user) return <div>No user found</div>;

return ( <div> <h1>{user.name}</h1> <p>{user.email}</p> <button onClick={execute}>Refresh</button>
<button onClick={reset}>Clear</button> </div> ); }
